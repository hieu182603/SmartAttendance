...
describe("Backend smoke", () => {
  ...
  test("Auth controller schema từ chối email rỗng", async () => {
    const { registerSchema } = await import(
      "../src/modules/auth/auth.controller.js"
    );
    const result = registerSchema.safeParse({ email: "", password: "abcdef", name: "Hi" });
    expect(result.success).toBe(false);
  });

  test("Auth controller schema chấp nhận payload hợp lệ", async () => {
    const { registerSchema } = await import(
      "../src/modules/auth/auth.controller.js"
    );
