...
async function createVerifiedUser(email = "user@test.com", password = "password123", name = "Test User") {
...
async function registerAndVerify(email = "user@test.com", password = "password123", name = "Test User") {
...
describe("TC-AUTH-003: Verify OTP success", () => {
  test("returns 200 and sets isVerified = true", async () => {
    ...
    await request(app).post("/api/auth/register").send({ email, password: "password123", name: "Verify User" });
...
describe("TC-AUTH-004: Verify OTP wrong code", () => {
  test("returns 400 for wrong OTP", async () => {
    ...
    await request(app).post("/api/auth/register").send({ email, password: "password123", name: "Wrong OTP" });
...
describe("TC-AUTH-005: Login success", () => {
  test("returns 200 with accessToken and refreshToken", async () => {
    await registerAndVerify("login@test.com", "password123");
...
describe("TC-AUTH-011: Expired trial user blocked", () => {
  test("returns 403 on /me when trial expired", async () => {
    ...
    const user = await UserModel.create({
      ...
      password: await hashed.hash("password123", 10),
      name: "Expired Trial",
      role: "TRIAL",
      isVerified: true,
      isActive: true,
