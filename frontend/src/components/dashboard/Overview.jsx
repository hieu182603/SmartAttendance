...
const infoCards = [
  { icon: Clock, color: 'accent-cyan', label: 'Ca làm việc', key: 'shift', delay: 0.5 },
  { icon: MapPin, color: 'success', label: 'Địa điểm', key: 'location', delay: 0.6 },
  { icon: Calendar, color: 'primary', label: 'Công tháng này', key: 'workingDays', delay: 0.7 },
]
        ...
        <div className="flex items-center justify-between relative z-10">
          ...
          >
            ...
            <p className="opacity-90 mt-2">
              Ca: {summary.shift?.timeRange || summary.shift?.label || summary.shift || '08:00 - 17:00'}
            </p>
          </motion.div>
        </div>
      </motion.div>
        ...
        <Card className="bg-[var(--surface)] border-[var(--border)] relative overflow-hidden">
          ...
          <CardContent className="p-8 relative z-10">
            <div className="text-center space-y-6">
              ...
              <div>
                <h2 className="text-2xl text-[var(--text-main)] mb-2">Chưa chấm công hôm nay</h2>
                <p className="text-[var(--text-sub)]">Quét mã QR tại văn phòng để điểm danh</p>
              </div>
              <motion.button 
              ...
              <Card className="bg-[var(--surface)] border-[var(--border)] hover:border-[var(--accent-cyan)] transition-all">
                <CardContent className="p-6">
                  <div className="flex items-center space-x-3">
                    ...
                    >
                      <item.icon className={`h-6 w-6 text-[var(--${item.color})]`} />
                    </motion.div>
                    <div>
                      <p className="text-sm text-[var(--text-sub)]">{item.label}</p>
      ...
      >
        <Card className="bg-[var(--surface)] border-[var(--border)]">
          ...
          <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: 'Lịch làm việc', icon: Calendar, page: 'schedule', color: 'accent-cyan' },
              { label: 'Yêu cầu nghỉ', icon: FileText, page: 'requests', color: 'warning' },
              { label: 'Lịch sử', icon: History, page: 'history', color: 'success' },
              { label: 'Số ngày phép', icon: CheckCircle2, page: 'leave-balance', color: 'primary' },
            ].map((action) => (
              <motion.button
                key={action.page}
                ...
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.9 + (action.page === 'schedule' ? 0 : action.page === 'requests' ? 0.1 : action.page === 'history' ? 0.2 : 0.3) }}
              >
                <action.icon className={`h-8 w-8 text-[var(--${action.color})] mb-2`} />
                <p className="text-sm text-[var(--text-main)]">{action.label}</p>
              </motion.button>
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="bg-[var(--surface)] border-[var(--border)]">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-[var(--text-main)]">Lịch sử gần đây</CardTitle>
              <button 
              ...
              <table className="w-full">
                ...
                <tbody>
                  {attendanceRows.length === 0 && !loading ? (
                    <tr>
                      <td colSpan={5} className="py-6 text-center text-sm text-[var(--text-sub)]">
